...
tinymce.PluginManager.add( 'fronted_image', function( editor ) {
	...
	function removeImage( node ) {
		...
		if( wrap === null && editor.dom.getParent(node, 'a') !== null ){
		...
		jQuery(editor.targetElm).find("p:empty").remove();
	...
	var resizing = {
		 ...
		 */
		get_closest_image_size : function(attachment_id, height, width, callback){
	...
	editor.on( 'init', function() {
		...
		editor.on('drop', function( event ) {
			if(!is_dragging && Modernizr.filereader && editor.settings.fronted_options.media_upload === true){
                        ...
                        fileReader.onload = function(evt){
                            jQuery.post(
                                global_vars.ajax_url,
                                {
                                    ...
                                    'post_id'               : editor.settings.fronted_options.post_id,
                                ...
                                function(response){
                                	var attachment = response.attachment_obj,
		...
		dom.bind( editor.getDoc(), 'dragend', function( event ) {
			is_dragging = false;
		});
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
            ...
            var class_match = node.className.match(/wp-image-\d+/);
            if(class_match !== null){
                var attachment_id = class_match[0].match(/\d+/)[0];
                resizing.get_closest_image_size(attachment_id, node.height, node.width, function(response){
                    if(response[3] === true){
                        node.className = node.className.replace(/size-\S+/, 'size-' + response[4]);
                        node.src = response[0];
                    }
                });
            }
